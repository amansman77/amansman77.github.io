...
- `Git`기반으로 `git-flow`전략을 사용하여 배포 환경을 구성하였습니다.
- `RestAPI`개발시 테스트코드의 작성을 독려하여 배포전 테스트를 수행하도록 구성하였습니다.
- `Jenkins`나 `Gitlab`을 활용하여 파이프라인(빌드/테스트/배포)을 구성한 경험이 있습니다.
- 필요에 따라 배포 액션을 `Delivery`, `Deploy` 중 선택적으로 활용합니다.
