...
안녕하세요.
최근 2년동안 AI플랫폼팀에서 leading 및 개발을 겸하고 있습니다. (2021년 기준 10년차)
업무 비중을 보면 team management(30%), AI플랫폼 lead(50%), 개발(20%) 수준으로 진행하고 있습니다.

업무 환경은 Bare metal 환경에서 kubernetes기반으로 MSA를 적용했습니다.
Gitlab을 활용하여 형상관리 및 CI/CD 를 구성했습니다.
근래에 활용하고 있는 언어는 Java, Python이고,
Java는 springboot기반으로 백엔드 트래픽을 받는데 사용하고 있고, Python은 데이터 처리 및 AI모델 추론 환경을 위해 사용하고 있습니다.
...
전자정부프레임워크를 3년정도 사용하면서 `Spring Web MVC`를 사용하였고, 이후 `RestAPI` 서버업무를 맡으면서 `Springboot`기반으로 `RestAPI`를 현재까지 개발을 진행하고 있습니다.

데이터 연계는 `Mybatis`, `JPA`를 활용하였고, `Swagger`나 `Restdocs`을 활용하여 `API spec`을 공유하고 있습니다.
인증관련하여 `JWT`를 사용하고 있습니다.

데이터베이스는 `Oracle`, `MariaDB`, `PostgreSQL`, `Elasticsearch`, `Redis`, `MongoDB`를 사용한 경험이 있지만 운영 경험을 한 것은 `Oracle`뿐입니다.

대표적인 제품 및 서비스로는 `GIS솔루션`(GIS서버, GIS데스크톱), `캠프 Camp`가 있고 현재 `AI플랫폼`을 개발중입니다.

### DevOps 개발자

`2018년 7월 2일`, `인라이플`에 입사하면서부터 팀내 개발 생산성을 높이는것을 목표로 `CI/CD` 자동화를 하다보니 현재까지(2021년기준 4년차) `DevOps`환경을 구성하고 사용하고 있습니다.

`DevOps` 환경은 다음 사항에 대해서 구성한 경험이 있습니다.
- `Git`기반으로 `git-flow`전략을 사용하여 형상관리 환경을 구성하였습니다.
- `Jenkins`나 `Gitlab`을 활용하여 CI/CD 환경을 구성했습니다.
- Release전 테스트 환경이 필요한 경우 stagging 환경을 구성하여 테스팅을 할 수 있도록 구성했습니다.
- Bara metal환경에서 Kubernetes기반으로 MSA를 적용했습니다.
